...
const userPreferences: { [userId: number]: string[] } = {
  2: ['Coffee', 'Tea'] // User 2 likes Coffee and Tea
};

const userPurchaseHistory: { [userId: number]: number[] } = {
...
router.get('/seasonal', (req, res) => {
  ...
  let seasonalProducts: Product[] = [];

  // Seasonal logic based on month
  if (currentMonth >= 3 && currentMonth <= 5) {
    // Spring - Fresh drinks
    seasonalProducts = products.filter(p => 
      p.category === 'Juice' || p.category === 'Smoothie'
    );
  } else if (currentMonth >= 6 && currentMonth <= 8) {
    // Summer - Cold drinks
    seasonalProducts = products.filter(p => 
      p.category === 'Smoothie' || p.category === 'Juice'
    );
...
// Helper functions
function getPersonalizedRecommendations(userId: number, limit: number): Product[] {
  ...
  let recommendedProducts = products.filter(p => 
    userPrefs.includes(p.category) && p.available
  );
...
function getTrendingProducts(limit: number): Product[] {
...
function getSimilarProducts(productId: number, limit: number): Product[] {
  const product = products.find(p => p.id === productId);
  if (!product) return [];
...
function getFrequentlyBoughtTogether(productId: number, limit: number): Product[] {
  // Mock data for frequently bought together
  const frequentlyBoughtMap: { [key: number]: number[] } = {
...
function getSimilarCategories(categories: string[]): string[] {
